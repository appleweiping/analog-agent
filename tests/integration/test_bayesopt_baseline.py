...
class BayesOptBaselineIntegrationTests(unittest.TestCase):
    def test_bayesopt_baseline_runs_inside_unified_suite(self) -> None:
        suite = run_ota_experiment_suite(
            steps=2,
            repeat_runs=1,
            modes=["bayesopt_baseline"],
            budget=ExperimentBudget(max_simulations=4, max_candidates_per_step=2),
            task_id="benchmark-ota2-bayesopt-ci",
        )

        self.assertEqual(suite.modes, ["bayesopt_baseline"])
        self.assertIsNotNone(suite.aggregated_stats)
        run = suite.runs[0]
        self.assertEqual(run.mode, "bayesopt_baseline")
        ...
        self.assertTrue(run.component_config.use_bayesopt_baseline)
    ...
    def test_bayesopt_baseline_emits_quick_truth_real_stats(self) -> None:
        suite = run_ota_experiment_suite(
            steps=2,
            repeat_runs=1,
            modes=["bayesopt_baseline", "full_system"],
            budget=ExperimentBudget(max_simulations=4, max_candidates_per_step=2),
            task_id="benchmark-ota2-bayesopt-stats-ci",
        )

        run = next(result for result in suite.runs if result.mode == "bayesopt_baseline")
