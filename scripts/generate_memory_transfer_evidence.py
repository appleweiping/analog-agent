...
def main() -> None:
    ...
    parser.add_argument(
        "--pair",
        choices=["ota_to_folded_cascode", "folded_cascode_to_ota", "ota_to_ldo", "ota_to_bandgap"],
        default="ota_to_folded_cascode",
    )
    ...
    elif args.pair == "folded_cascode_to_ota":
        suite = run_folded_cascode_to_ota_memory_transfer_suite(
            source_episodes=args.source_episodes,
            target_episodes=args.target_episodes,
            max_steps=args.steps,
        )
        output_root = args.output_root / "ota2_v1"
    elif args.pair == "ota_to_ldo":
