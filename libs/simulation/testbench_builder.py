...
from libs.vertical_slices.bandgap_spec import bandgap_v1_measurement_contract_path, bandgap_v1_testbench_path
...
def _load_bandgap_analysis_plan(fidelity_level: str) -> tuple[list[AnalysisStatement], str, list[str]]:
    return _load_family_analysis_plan(bandgap_v1_testbench_path(fidelity_level))


def _build_bandgap_measurement_contract(analysis_plan: AnalysisPlan) -> MeasurementContract:
    return _build_family_measurement_contract(bandgap_v1_measurement_contract_path(), analysis_plan)
...
def build_analysis_plan(task: DesignTask, request: SimulationRequest) -> AnalysisPlan:
    ...
    real_bandgap_native = request.backend_preference == "ngspice" and task.circuit_family == "bandgap" and task.topology.topology_mode == "fixed"
    ...
    elif real_bandgap_native and fidelity_level in {"quick_truth", "focused_truth"}:
        ordered, execution_policy, termination_rules = _load_bandgap_analysis_plan(fidelity_level)
...
def build_measurement_contract(task: DesignTask, analysis_plan: AnalysisPlan) -> MeasurementContract:
    ...
    if task.circuit_family == "bandgap" and task.topology.topology_mode == "fixed":
        return _build_bandgap_measurement_contract(analysis_plan)
