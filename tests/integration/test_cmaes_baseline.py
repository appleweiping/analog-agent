...
class CmaesBaselineIntegrationTests(unittest.TestCase):
    def test_cmaes_baseline_runs_inside_unified_suite(self) -> None:
        suite = run_ota_experiment_suite(
            steps=2,
            repeat_runs=1,
            modes=["cmaes_baseline"],
            budget=ExperimentBudget(max_simulations=4, max_candidates_per_step=2),
            task_id="benchmark-ota2-cmaes-ci",
        )

        self.assertEqual(suite.modes, ["cmaes_baseline"])
        self.assertIsNotNone(suite.aggregated_stats)
        run = suite.runs[0]
        self.assertEqual(run.mode, "cmaes_baseline")
        ...
        self.assertTrue(run.component_config.use_cmaes_baseline)
    ...
    def test_cmaes_baseline_emits_quick_truth_real_stats(self) -> None:
        suite = run_ota_experiment_suite(
            steps=2,
            repeat_runs=1,
            modes=["cmaes_baseline", "full_system"],
            budget=ExperimentBudget(max_simulations=4, max_candidates_per_step=2),
            task_id="benchmark-ota2-cmaes-stats-ci",
        )

        run = next(result for result in suite.runs if result.mode == "cmaes_baseline")
